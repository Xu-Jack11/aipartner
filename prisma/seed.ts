...
const main = async () => {
  ...
  const sessionLinear = await upsertSession(
    user.id,
    "线性代数重点回顾",
    "线性代数"
  );
  const sessionEnglish = await upsertSession(
    user.id,
    "阅读理解强化练习",
    "考研英语"
  );
  const sessionStatistics = await upsertSession(
    user.id,
    "概率论章节复习",
    "概率统计"
  );
...
const upsertSession = (userId: string, title: string, focus: string) => {
...
type PlanSeedInput = {
  ...
  tasks: { summary: string; status: string; dueDate: Date }[];
};
